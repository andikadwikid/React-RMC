...
export const useDashboardState = (): [DashboardState, DashboardActions] => {
  ...
  const initialPerformancePeriod = useMemo(
    () => detectBestPerformancePeriod(),
    [],
  );
  const initialRiskPeriod = useMemo(() => detectBestRiskPeriod(), []);
  const initialGeographicPeriod = useMemo(
    () => detectBestGeographicPeriod(),
    [],
  );
  const initialRiskCapturePeriod = useMemo(
    () => detectBestRiskCapturePeriod(),
    [],
  );
  const initialInvoiceStatusPeriod = useMemo(
    () => detectBestInvoiceStatusPeriod(),
    [],
  );
  const initialAgingReceivablesPeriod = useMemo(
    () => detectBestAgingReceivablesPeriod(),
    [],
  );
  ...
  const [selectedPerformancePeriod, setSelectedPerformancePeriod] =
    useState<DataPeriod>(initialPerformancePeriod);
  const [performanceAutoSelected, setPerformanceAutoSelected] = useState(true);

  // Risk state
  const [selectedRiskPeriod, setSelectedRiskPeriod] =
    useState<RiskDataPeriod>(initialRiskPeriod);
  const [riskAutoSelected, setRiskAutoSelected] = useState(true);

  // Geographic state
  const [selectedGeographicPeriod, setSelectedGeographicPeriod] =
    useState<GeographicDataPeriod>(initialGeographicPeriod);
  ...
  const [selectedRiskCapturePeriod, setSelectedRiskCapturePeriod] =
    useState<RiskCaptureDataPeriod>(initialRiskCapturePeriod);
  ...
  const [selectedInvoiceStatusPeriod, setSelectedInvoiceStatusPeriod] =
    useState<InvoiceStatusDataPeriod>(initialInvoiceStatusPeriod);
  ...
  const [selectedAgingReceivablesPeriod, setSelectedAgingReceivablesPeriod] =
    useState<AgingReceivablesDataPeriod>(initialAgingReceivablesPeriod);
  ...
  const handleGeographicPeriodChange = useCallback(
    (period: GeographicDataPeriod) => {
  ...
  const handleRiskCapturePeriodChange = useCallback(
    (period: RiskCaptureDataPeriod) => {
  ...
  const handleInvoiceStatusPeriodChange = useCallback(
    (period: InvoiceStatusDataPeriod) => {
  ...
  const handleAgingReceivablesPeriodChange = useCallback(
    (period: AgingReceivablesDataPeriod) => {
