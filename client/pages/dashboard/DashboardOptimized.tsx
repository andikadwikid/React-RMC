...
const ProjectDistributionChart = lazy(() => import("@/components/ProjectDistributionChart"));
const RiskCapturePieChart = lazy(() => import("@/components/RiskCapturePieChart"));
      ...
      <CardContent className="p-4 sm:p-6">
        <div className="flex items-center justify-between">
          <div className="flex-1">
            ...
            <p className="text-2xl sm:text-3xl font-bold">{projectSummary.completed}</p>
...
const PerformanceChartSection = memo(({ 
  ...
  const showFallback = useFallbackMessage(performanceAutoSelected, selectedPerformancePeriod);
          ...
          <div className="flex items-center gap-2 w-full lg:w-auto">
            <div className="flex-1">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-2">
                <p className="text-sm text-gray-600">{selectedPerformancePeriod.label}</p>
                <Badge
                  variant="secondary"
                  className={`w-fit ${
                    selectedPerformancePeriod.type === "yearly"
...
const DashboardOptimized = () => {
  ...
  // Memoized insights
  const performanceInsights = usePerformanceInsights(state.selectedPerformancePeriod);
  const riskInsights = useRiskInsights(state.riskCategories);
  const geographicInsights = useGeographicInsights(state.provinceData);
  const { invoiceInsights, agingInsights } = useFinancialInsights(state.invoiceStatus, state.agingReceivables);
  const riskCaptureInsights = useRiskCaptureInsightsData(state.riskCaptureData);

  // Memoized fallback messages
  const riskFallback = useFallbackMessage(state.riskAutoSelected, state.selectedRiskPeriod);
  const geographicFallback = useFallbackMessage(state.geographicAutoSelected, state.selectedGeographicPeriod);
  const riskCaptureFallback = useFallbackMessage(state.riskCaptureAutoSelected, state.selectedRiskCapturePeriod);
              ...
              <div className="flex items-center gap-2">
                <div>
                  ...
                  <div className="flex items-center my-2">
                    <p className="text-sm text-gray-600 mt-1">{state.selectedRiskPeriod.label}</p>
                    <Badge
                      variant="secondary"
                      className={`ml-2 ${
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <div className={`w-2 h-2 rounded-full ${getStatusColor("overdue")}`}></div>
                            <span className="text-xs text-gray-600">Overdue</span>
                          </div>
                          <span className="text-xs font-medium text-red-600">{category.overdue}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <div className={`w-2 h-2 rounded-full ${getStatusColor("inProcess")}`}></div>
                            <span className="text-xs text-gray-600">In Process</span>
                          </div>
                          <span className="text-xs font-medium text-yellow-600">{category.inProcess}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <div className={`w-2 h-2 rounded-full ${getStatusColor("closed")}`}></div>
                            <span className="text-xs text-gray-600">Closed</span>
                          </div>
                          <span className="text-xs font-medium text-green-600">{category.closed}</span>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="flex flex-col md:flex-row items-start justify-between gap-5">
              <div className="flex items-center gap-2">
                <div>
                  ...
                  <div className="flex items-center my-2">
                    <p className="text-sm text-gray-600">{state.selectedGeographicPeriod.label}</p>
                    <Badge
                      variant="secondary"
                      className={`ml-2 ${
              ...
              <div className="pt-4 border-t">
                <div className="text-sm text-gray-600">
                  <div className="flex justify-between mb-2">
                    ...
                    <span className="font-semibold">{state.provinceData.length}</span>
                  </div>
                  <div className="flex justify-between mb-2">
                    <span>Total Pendapatan:</span>
                    <span className="font-semibold text-green-600">
                      {formatCurrency(state.provinceData.reduce((sum: number, p: any) => sum + p.revenue, 0))}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-green-600">
                      {formatCurrency(
                        Math.round(
                          state.provinceData.reduce((sum: number, p: any) => sum + p.revenue, 0) / state.provinceData.length,
                        ),
                      )}
                    </span>
                  </div>
              ...
              <div className="flex items-center gap-2">
                <div>
                  <div className="flex gap-3">
                    ...
                    <CardTitle className="flex items-center gap-2">Risk Capture Distribution</CardTitle>
                  </div>
                  <div className="flex items-center my-2">
                    <p className="text-sm text-gray-600 mt-1">
                      {state.selectedRiskCapturePeriod.label} - Distribusi level risiko berdasarkan severity assessment
                    </p>
                    <Badge
                      variant="secondary"
                      className={`ml-2 ${
              ...
              <div className="flex justify-between gap-5 mt-5">
                <div className="bg-red-50 p-3 rounded-lg">
                  ...
                  <div className="text-lg font-bold text-red-600">
                    {riskCaptureInsights.highRiskItems} ({riskCaptureInsights.highRiskPercentage}%)
                  </div>
                </div>
                <div className="bg-green-50 p-3 rounded-lg">
                  <div className="text-sm font-medium text-green-800">Low Risk</div>
                  <div className="text-lg font-bold text-green-600">
                    {riskCaptureInsights.lowRiskItems} ({riskCaptureInsights.lowRiskPercentage}%)
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 space-y-2">
              <div className="text-sm text-gray-600">
                <div className="flex gap-5 mb-1">
                  ...
                  <span className="font-semibold">{riskCaptureInsights.totalItems}</span>
                </div>
