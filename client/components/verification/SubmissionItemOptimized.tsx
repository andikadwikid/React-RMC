      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 text-sm">
          ...
          <div className="flex items-start gap-2 p-2 sm:p-3 bg-blue-50 rounded-lg border border-blue-100">
            ...
            <div className="min-w-0 flex-1">
              <span className="block text-xs font-medium text-blue-700 mb-1">Submitter</span>
              <span className="text-blue-900 font-medium break-words">{submission.submittedBy}</span>
            </div>
          </div>
          ...
          <div className="flex items-start gap-2 p-2 sm:p-3 bg-green-50 rounded-lg border border-green-100">
            ...
            <div className="min-w-0 flex-1">
              <span className="block text-xs font-medium text-green-700 mb-1">Submit Date</span>
          ...
          {submission.verifierName && (
            <div className="flex items-start gap-2 p-2 sm:p-3 bg-purple-50 rounded-lg border border-purple-100">
              ...
              <div className="min-w-0 flex-1">
                <span className="block text-xs font-medium text-purple-700 mb-1">Verifier</span>
                <span className="text-purple-900 font-medium break-words">{submission.verifierName}</span>
              </div>
            </div>
          )}
          ...
          <div className="hidden sm:flex items-start gap-2 p-2 sm:p-3 bg-gray-50 rounded-lg border border-gray-100">
            ...
            <div className="min-w-0 flex-1">
              <span className="block text-xs font-medium text-gray-700 mb-1">Project ID</span>
              <span className="text-gray-900 font-mono text-xs break-all">{submission.projectId}</span>
            </div>
          </div>
        </div>
        ...
        {submission.items && submission.items.length > 0 && (
          <div className="flex items-center gap-2 p-2 sm:p-3 bg-yellow-50 rounded-lg border border-yellow-100">
            ...
            <span className="text-sm font-medium text-yellow-700">Total Items:</span>
            <Badge variant="outline" className="font-semibold text-yellow-800 border-yellow-300">
            ...
            {submission.items.some(item => item.verifierStatus) && (
              <div className="ml-auto text-xs text-yellow-700">
                Verified: {submission.items.filter(item => item.verifierStatus === 'lengkap').length}/{submission.items.length}
              </div>
            )}
          </div>
        )}
        ...
        {submission.overallComment && (
          <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-start gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-blue-900">Comment:</span>
            </div>
        ...
        <div className="sm:hidden border-t pt-3 mt-1">
          <div className="flex items-center justify-between text-xs text-gray-500">
            <span>Status: {submission.status.replace('_', ' ').toUpperCase()}</span>
            {submission.items && (
