...
const totalDays = Math.floor((new Date(project.endDate) - new Date(project.startDate)) / (1000 * 60 * 60 * 24))
const timeElapsedPercentage = (daysElapsed / totalDays) * 100
...
const schedulePerformance = project.progress >= timeElapsedPercentage ? 'on-track' : 'delayed'
const budgetPerformance = budgetUsedPercentage <= project.progress ? 'under-budget' : 'over-budget'
```
